            ...
            <div className="glass-panel h-full flex flex-col py-6">
                ...
                <nav className="flex-1 px-4 space-y-2">
                    {NAV_ITEMS.map(({ path, icon: Icon, label }) => {
                        const isActive = location.pathname === path || (path !== '/jobs' && location.pathname.startsWith(path));
                        return (
                            <NavLink
                                key={path}
                                to={path}
                            ...
                            >
                                <div className={`absolute inset-0 transition-opacity duration-300 ${isActive ? 'bg-primary/20 opacity-100' : 'bg-white/5 opacity-0 group-hover:opacity-100'}`} />
                                {isActive && (
                                    <motion.div
